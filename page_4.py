...
# Zoek het schedule-bestand (naam bevat "schedule" of "airport", case-insensitief)
schedule_key = None
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Verwijder rijen zonder vertraging
    df = df.dropna(subset=["vertraging_min"])

    # ── 2. Vertragingscategorie ──────────────────────────────────────────
    def categorize(v):
        if v < 5:
            return "On time"
        elif v <= 45:
    ...
    # ── 8. Bestemming/afkomst (top-N encoded) ────────────────────────────
...
st.markdown("Vul de waarden in om een voorspelling te doen met het getrainde model.")
...
for label in gekozen_labels:
    ...
    with cols_pred[col_map[label]]:
        if col_key == "uur":
            ...
            pred_input[col_key] = st.number_input(label, 1, 12, 6)
